...
class WXYD:
    ...
    def info(self):
        ...
        p = {"code": "5G7YUK23J", "un": self.un, "token": self.token, "pageSize": 20}
    ...
    def testCheck(self, a, url):
        ...
        if checkDict.get(a[4]) != None:
            ...
            for i in range(60):
                if i % 30 == 0:
                    JhWxPusher.push(self.cg['name'], url, a[3], 'zhyd', self.cg['uids'],self.key)
    ...
    def submit(self):
        u = f'http://u.cocozx.cn/api/{self.bz}/submit?zx=&xz=1'
        p = {"un": self.un, "token": self.token, "pageSize": 20}
        r = requests.post(u, headers=self.headers, json=p)
        print(self.name, r.text)
        rj = r.json()
    ...
    def wdmoney(self):
        if self.moneyCurrent < config.ybtxbz:
            ...
            return False
        elif 3000 <= self.moneyCurrent < 5000:
            txm = 3000
        elif 5000 <= self.moneyCurrent < 10000:
            txm = 5000
